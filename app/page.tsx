    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-20">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between gap-4">
          ...
          {/* Right side: new upload + sign out */}
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <span className="text-xs text-gray-400 hidden md:block truncate max-w-[150px]">
              {user.email}
            </span>
            <button
              onClick={signOut}
            ...
            {productionDate && (
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full
                  ...
                  Production date: {new Date(productionDate + 'T00:00:00').toLocaleDateString('en-AU', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                </span>
                {recipeRunId && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full
                                   bg-emerald-50 border border-emerald-200 text-xs text-emerald-700">
                    Recipes auto-queued — check Recipes tab
                  </span>
                )}
              </div>
            )}
